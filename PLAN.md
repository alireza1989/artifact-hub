...
- [x] **Copy-paste MCP config panel**: `/connect` page with the exact remote-connector URL and the `claude_desktop_config.json` / `mcp-remote` snippet per §4.2, copy buttons, placeholder-by-default token handling with owner-only explicit reveal
- [x] **Re-run AI on an existing artifact**: owner button to regenerate metadata suggestions (badge mechanism reused) and to force feedback re-synthesis — both through existing guardrails/telemetry; unit + integration tests
...
| 2026-07-07 | Raw `db.execute` results must go through `rowsOf()` (dual-driver shape bug) | `getDb()` picks the Neon serverless driver for `*.neon.tech` and postgres.js otherwise. Query-builder results are uniform, but `db.execute(sql\`...\`)` returns an iterable RowList on postgres.js vs node-postgres `{rows}` on Neon — so spreading the result passed every (postgres.js-backed) test and crashed only on prod: `/admin/tags` and `hub_stats` both hit it. Added `rowsOf()` normalizer in `@/db` (unit-tested with both shapes), fixed both call sites, verified `listTagUsage` + `hubStats` against the real Neon driver. Convention: every future `execute()` SELECT unwraps via `rowsOf` — documented in README §Maintaining. |
| 2026-07-07 | MCP Desktop config uses the `Authorization:${"$"}{AUTH_HEADER}` env-var pattern | Claude Desktop passes args verbatim (no `${"$"}{}` interpolation) and mishandles spaces inside args; mcp-remote substitutes env vars in header args itself and documents exactly this no-space pattern. `lib/mcp-snippets`, `/connect`, and the README snippet all updated; the demo team token is published in README/WALKTHROUGH by explicit user decision (demo deployment, no RBAC). |
...
| 2026-07-07 | **Whole web UI team-gated** (user decision) — every page + `/raw/[id]` requires the unlocked session; only `/share/[token]`, `/unlock`, and REST/MCP *reads* stay public | Browsing was open by design (PLAN §3.4 gated writes only), but the user wants the hub private to the team. Pages gate server-side via the existing `/unlock` session cookie (`?next=` returns the visitor, open-redirect-guarded); `/raw` accepts session cookie, admin bearer, or a `?st=<share-token>` scoped to exactly that artifact (`countAccess:false` so asset fetches don't inflate view counts) — that's how sessionless share viewers still load embedded content. REST/MCP reads deliberately stay open so the unauthenticated Desktop-connector flow keeps working (see MCP OAuth deferral); note the data itself therefore remains readable via API — page gating is a UX boundary for the web surface, plus real gating of raw bytes. |
