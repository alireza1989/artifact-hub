...
import { z } from "zod";
import { createArtifact, fetchSourceBytes } from "@/core/artifacts";
...
const DESCRIPTION =
  ...
  "anything you omit is left blank for now (automatic metadata suggestions ship in a later " +
  "release — the returned `aiFilled` lists AI-generated fields and is currently always empty), " +
  "so prefer supplying at least a title. Returns the new artifact's `id` (use it with " +
  "create_share_link, get_artifact, add_comment), its detected `kind`, stored size, and a gallery " +
  "URL. Requires the team bearer token. After publishing, confirm the saved title and tags with " +
  "the user.";
...
const outputSchema = {
  ...
  aiFilled: z
    .array(z.string())
    .describe("Metadata fields generated by AI. Always empty until synthesis ships (Phase 4)."),
};
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Published "${artifact.title}" (${artifact.kind}, id ${artifact.id}). Confirm the title and tags with the user.`,
            },
          ],
          structuredContent,
        };
